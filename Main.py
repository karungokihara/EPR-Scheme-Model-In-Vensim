...
model_path = 'Policy1and2_V11.mdl'
...
for scenario in scenarios:
    ...
    initial_values = {
        "policy1 introduction time": 2043, # DRS PS BOTTLE
        "policy2 introduction time": 2043, # EPR 2 A) All film applications
        "policy3 introduction time": 2043, # EPR 2B) PS APPLICATION
        "policy4 introduction time": 2027  # EPR 2C) ALL PVC
    }


# I have reduced the delay to take effect in the current year of introduction with some factor instead of pushing directly to the next year


    for key, value in initial_values.items():
        if "introduction time" in key:
            initial_values[key] -= 1 
    ...
    # Replacements
    model['material'] = model['material'].replace({'pe': 'PE', 'ps': 'PS', 'idpe': 'LDPE', 'pvc': 'PVC', 'ldpe': 'LDPE', 'other': 'Other', 'pet': 'PET', 'pp': 'PP', 'hdpe': 'HDPE'})
...
string_list = ['Total POM', 'littering','sent for sorting', 'Mechanical recycling']
...
if not total_model_output.empty:
    total_model_output.to_csv('EPR2CSSS.csv', index=False)
